...
export default class ArrowAggregator implements PartAggregator {
  ...
  aggregatePartResponse(partNumber: string): PartialAggregatedPart {
    ...
    if (
      this.partsResponse.totalRecords > 0 &&
      this.partsResponse.pricingResponse.length > 0
    ) {
      ...
      partialAggregatedPart.name =
        this.partsResponse.pricingResponse[0].arwPartNum.name;
      partialAggregatedPart.description =
        this.partsResponse.pricingResponse[0].description;
      partialAggregatedPart.manufacturerName =
        this.partsResponse.pricingResponse[0].manufacturer;

      // set specifications found in MyArrow part data
      partialAggregatedPart.specifications.htsCode =
        this.partsResponse.pricingResponse[0].htsCode || undefined;
      partialAggregatedPart.specifications.euRohs =
        this.partsResponse.pricingResponse[0].euRohs || undefined;
      partialAggregatedPart.specifications.chinaRohs =
        this.partsResponse.pricingResponse[0].chinaRohs || undefined;
      partialAggregatedPart.specifications.partClassification =
        this.partsResponse.pricingResponse[0].partClassification || undefined;
      partialAggregatedPart.specifications.exportControlClassificationNumberUS =
        this.partsResponse.pricingResponse[0]
          .exportControlClassificationNumberUS || undefined;
      partialAggregatedPart.specifications.exportControlClassificationNumberWAS =
        this.partsResponse.pricingResponse[0]
          .exportControlClassificationNumberWAS || undefined;
      partialAggregatedPart.specifications.category =
        this.partsResponse.pricingResponse[0].taxonomy || undefined;
      partialAggregatedPart.specifications.SVHC =
        this.partsResponse.pricingResponse[0].SVHC.svhcOverThreshold ||
        undefined;
    }
    this.partsResponse.pricingResponse.forEach((part) => {
      ...
      if (part.leadTime) {
        ...
        partialAggregatedPart.manufacturerLeadTime = Math.min(
          ...
          part.leadTime.supplierLeadTime * 7,
        );
      }
      ...
      if (
        ...
        !partialAggregatedPart.specifications.countryOfOrigin
      ) {
        partialAggregatedPart.specifications.countryOfOrigin =
          part.countryOfOrigin;
      }
  ...
  buildPackaging(part: ArrowPart): Packaging {
    ...
    const priceBreaks = part.pricingTier
      ? this.buildPricing(part.pricingTier)
      : part.resalePrice
      ? [
          {
            breakQuantity: 1,
    ...
    if (part.leadTime) {
      packaging.manufacturerLeadTime = `${parseLeadTimeDays(
        part.leadTime.supplierLeadTime.toString(),
      )}`;
    }
    return packaging;
  }
