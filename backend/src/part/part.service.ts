...
export class PartService {
  ...
  aggregateParts(
        ...
        if (cur.sourcePart == SUPPLIER_NAMES.TTI) {
          ...
          return {
            ...
            specifications: {
              ...prevPart.specifications,
              ...cur.specifications,
              //only use arrow category if not previously set
              category:
                prevPart.specifications.category || cur.specifications.category,
            },
            ...
            productImageUrl: prevPart.productImageUrl || cur.productImageUrl,
          };
        }
      },
      {
    ...
    Object.entries(aggregatedData.specifications).forEach(([k, v]) => {
      ...
      if (
        k == 'rohsStatus' &&
        aggregatedData.specifications.euRohs &&
        aggregatedData.specifications.chinaRohs
      ) {
        return;
      }
    ...
    if (
      ...
      throw new InternalServerErrorException(
        'Could not find enough data on this part',
      );
    }
  ...
  async getPart(partNumber: string): Promise<AggregatedPart> {
    ...
    // get ttiPart
    const retrievedParts: PartResponses = {};
    try {
      // Talking to a real API we would page this request until the
      // current page number == total number pages
    ...
    const aggregatedPart: AggregatedPart = this.aggregateParts(
      retrievedParts,
      partNumber,
    );
