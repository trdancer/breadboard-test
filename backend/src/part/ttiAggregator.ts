import { Packaging, PartAggregator, PartialAggregatedPart, PriceBreak, TTIPart, TTIPartResponse, TTIPricingBreak } from "@types";
import { SUPPLIER_NAMES } from "../helpers/constants";
import { parseLeadTimeDays } from "../helpers/utils";
import { InternalServerErrorException } from "@nestjs/common";
export default class TTIAggregator implements PartAggregator<TTIPartResponse> {
    ...
    aggregatePartResponse(partNumber:string): PartialAggregatedPart {
        ...
        const filteredTTIParts = this.partsResponse.parts.filter((part) => part.ttiPartNumber == partNumber)
        ...
        if (filteredTTIParts.length > 0) {
            ...
            // set specifications
            partialAggregatedPart.specifications.category = filteredTTIParts[0].category
            partialAggregatedPart.specifications.eccn = filteredTTIParts[0].exportInformation.eccn
            partialAggregatedPart.specifications.leadInTerminals = filteredTTIParts[0].environmentalInformation.leadInTerminals
            partialAggregatedPart.specifications.reachSubstanceName = filteredTTIParts[0].environmentalInformation.reachSubstanceName
            partialAggregatedPart.specifications.htsCode = filteredTTIParts[0].hts
            partialAggregatedPart.specifications.rohsStatus = filteredTTIParts[0].environmentalInformation.rohsStatus
            partialAggregatedPart.specifications.SVHC = filteredTTIParts[0].environmentalInformation.reachSVHC

        }
        
        filteredTTIParts.forEach((part) => {
            console.log('processing tti part...')
            // availability info
            try {
              const parsedLeadTime = parseLeadTimeDays(part.leadTime)
              partialAggregatedPart.manufacturerLeadTime = Math.min(partialAggregatedPart.manufacturerLeadTime, parsedLeadTime)
            } catch (err) {
              console.log(err)
            ...
            if (!partialAggregatedPart.specifications?.taric && part.exportInformation.taric) {
              partialAggregatedPart.specifications.taric = part.exportInformation.taric
            }
            // build packaging
